...
void onRequest(const HttpRequest& req, HttpResponse* resp) {
  ...
  if (it != redirections.end()) {
    resp->setStatusCode(HttpResponse::HttpStatusCode::k301MovedPermanently);
    ...
    for (auto it = redirections.cbegin(); it != redirections.cend(); ++it) {
      text.append("<ul>" + it->first + " =&gt; " + it->second + "</ul>");
    }
    resp->setBody(
...
int main(int argc, char* argv[]) {
  redirections["/1"] = "https://hacker-cube.com";
  redirections["/2"] = "https://baidu.com";
  int numThreads = 0;
  if (argc > 1) {
    benchmark = true;
